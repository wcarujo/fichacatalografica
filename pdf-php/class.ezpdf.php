...
// 
// version 006 (versioning is linked to class.pdf.php)
//
...
function ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$gap,$size,&$y){
  // uses ezText to add the text, and returns the height taken by the largest heading
  $mx=0;
  foreach($cols as $colName=>$colHeading){
    $this->ezSetY($y);
    $this->ezText($colHeading,$size,array('aleft'=> $pos[$colName],'aright'=>($maxWidth[$colName]+$pos[$colName])));
    $dy=$y-$this->y;
    if ($dy>$mx){
      $mx=$dy;
    }
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  // 'cols' => array(<colname>=>array('justification'=>'left','width'=>100),<colname>=>....)
  ...
  // if the width is set, then adjust the table to that width
  if ($options['width'] && $adjustmentWidth>0 && $setWidth<$options['width']){
    $factor = ($options['width']-$setWidth)/($adjustmentWidth);
    $dx=0;
    $lastp=$x;
    $dolast=0;
    foreach($pos as $colName=>$p){
      if ($dolast){
        $dx += ($p-$lastp-$options['gap'])*($factor-1);
        $dolast=0;
      }
      $pos[$colName]+=$dx;
      $lastp = $p;
    ...
    foreach($maxWidth as $colName=>$p){
      if (!isset($options['cols'][$colName]) || !isset($options['cols'][$colName]['width']) || $options['cols'][$colName]['width']<=0){
        $maxWidth[$colName]=$factor*$maxWidth[$colName];
      }
    }
    $t=$x+$options['width'];
  }
...
//  $y0=$y+$height+$decender;
  ...
  if ($options['showHeadings']){
    $mx = $this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$options['gap'],$options['fontSize'],$y);
    $dy=$options['gap']/2;
    $y -= ($mx-$height);
  }
  $y1 = $y+$decender+$dy;
